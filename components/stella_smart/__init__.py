...
async def to_code(config):
    for conf in config:
        ...
        cg.add(var.set_mac_address(conf[CONF_MAC_ADDRESS].as_hex))
        cg.add(var.set_device_name(conf[CONF_NAME]))

        if CONF_BATTERY in conf:
            sens = await sensor.new_sensor(conf[CONF_BATTERY])
            cg.add(var.set_battery_sensor(sens))
        if CONF_SPRAY_COUNT in conf:
            sens = await sensor.new_sensor(conf[CONF_SPRAY_COUNT])
            cg.add(var.set_spray_count_sensor(sens))
        if CONF_POWER in conf:
            sw = await switch.new_switch(conf[CONF_POWER])
            cg.add(sw.set_parent(var))
            cg.add(var.set_power_switch(sw))
        if CONF_TRIGGER_SPRAY in conf:
            btn = await button.new_button(conf[CONF_TRIGGER_SPRAY])
            cg.add(btn.set_parent(var))
            cg.add(var.set_spray_button(btn))
        if CONF_RESET_SPRAY in conf:
            btn = await button.new_button(conf[CONF_RESET_SPRAY])
            cg.add(btn.set_parent(var))
            cg.add(var.set_reset_button(btn))
        if CONF_SPRAY_DURATION in conf:
            sel = await select.new_select(
                conf[CONF_SPRAY_DURATION],
                options=["10 min", "20 min", "40 min"],
            )
            cg.add(sel.set_parent(var))
            cg.add(var.set_duration_select(sel))
        if CONF_TIMER_MODE in conf:
            sel = await select.new_select(
                conf[CONF_TIMER_MODE],
                options=["X", "6h", "8h", "12h"],
            )
            cg.add(sel.set_parent(var))
            cg.add(var.set_timer_select(sel))
        if CONF_FIRMWARE_VERSION in conf:
            ts = await text_sensor.new_text_sensor(conf[CONF_FIRMWARE_VERSION])
